...
import { Trophy, Medal, Sparkles, TrendingUp, TrendingDown, Minus, Calendar, Users, Award } from 'lucide-react'
...
export default function AdminLeaderboardPage() {
  ...
  const totalUnits = (leaderboard || []).reduce((sum, e) => sum + Number(e.total_units || 0), 0)
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Leaderboard"
        description="Employee points ranking and performance across any timeframe"
      />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {top3.map((emp, i) => {
            ...
            return (
              <div
                key={emp.employee_id}
                className={cn("relative rounded-xl border p-5 flex flex-col justify-between transition-all", podiumStyles[i])}
              >
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-bold text-zinc-900 truncate">{emp.full_name}</p>
      ...
      {/* Pure Points Aggregate Stat Cards */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard title="Total Participants" value={leaderboard?.length || 0} icon={Users} subtitle="Active Employees" />
        <StatCard title="Total Points Earned" value={totalPoints.toLocaleString()} icon={Sparkles} subtitle={getTimeframeLabel()} />
        <StatCard title="Avg Points / Employee" value={avgPoints.toLocaleString()} icon={Medal} subtitle="Average performance" />
        <StatCard
          title="Top Performer"
          value={top3[0]?.full_name || 'N/A'}
          icon={Trophy}
          subtitle={`${Number(top3[0]?.total_points || 0).toLocaleString()} pts earned`}
        />
      </div>

      {/* Full Rankings Table (Points Only) */}
      <Card>
        <CardHeader>
          <CardTitle className="text-sm font-semibold text-zinc-900">
            Points Rankings ({getTimeframeLabel()} — {leaderboard?.length || 0} Employees)
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="py-16 text-center text-sm text-zinc-500">
              No points recorded for the selected timeframe.
            </div>
          ) : (
            <Table>
              <TableHeader>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2.5">
                          ...
                          <div className="flex flex-col min-w-0">
                            <span className="text-sm font-semibold text-zinc-900 truncate">{emp.full_name}</span>
