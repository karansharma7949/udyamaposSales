    ...
    try {
      ...
      if (role !== 'admin') {
        toast.error('Access Denied: This account does not have administrator privileges.', {
          description: 'Please use the Employee Portal instead.',
        })
      ...
      toast.success(`Welcome back, ${profile?.full_name || cleanEmail}`)
      router.push('/admin/dashboard')
    } catch (err) {
      // User credential error - show friendly toast without triggering Next.js dev overlay
    ...
    <div className="min-h-screen flex items-center justify-center bg-zinc-50 px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-bold text-zinc-900 tracking-tight">UdyamaPOS Admin</h1>
          <p className="text-sm text-zinc-500 mt-1">Sign in to the administrator panel</p>
        </div>
        ...
        <div className="text-center mt-5">
          ...
          >
            Not an admin? Go to Employee Portal →
          </Link>
        </div>
      </div>
    </div>
