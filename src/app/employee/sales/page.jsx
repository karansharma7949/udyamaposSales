...
export default function EmployeeSalesPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <Dialog open={saleOpen} onOpenChange={handleOpenSaleDialog}>
            ...
            <DialogContent className="w-[95vw] sm:max-w-md max-h-[92vh] overflow-y-auto p-4 sm:p-6 border-zinc-200 bg-white shadow-xl rounded-2xl">
              <DialogHeader>
                ...
                <DialogDescription className="text-xs text-zinc-500">
                  Record a sold item to earn points towards your target.
                </DialogDescription>
              </DialogHeader>
              <SaleForm
                key={formKey}
                onSubmit={async (data) => {
                  try {
                    ...
                    if (!e.message?.includes('Target exceeded')) {
      ...
      {/* Search Bar */}
      <div className="p-3 sm:p-3.5 bg-white rounded-xl border border-zinc-200 shadow-xs">
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="divide-y divide-zinc-100 sm:hidden">
                {sales.map((sale) => {
                  const soldPrice = Number(sale.sold_at_price || (sale.total_amount && sale.quantity ? sale.total_amount / sale.quantity : 0))
                  const totalAmt = Number(sale.total_amount || soldPrice * Number(sale.quantity || 1))

                  return (
                    <div key={sale.id} className="p-3.5 space-y-2 hover:bg-zinc-50/70 transition-colors">
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          <p className="font-semibold text-zinc-900 text-xs leading-snug truncate">
                            {sale.products?.product_name || 'Product'}
                          </p>
                          <p className="text-[10px] text-zinc-400 truncate">{sale.products?.product_code || ''}</p>
                        </div>
                      ...
                      <div className="flex items-center justify-between text-xs text-zinc-600">
                        <span>
                          {sale.quantity} {sale.quantity === 1 ? 'unit' : 'units'} × ₹{soldPrice.toLocaleString()}
                        </span>
                      ...
                      <div className="flex items-center justify-between pt-1 border-t border-zinc-50 text-[11px] text-zinc-400">
                        ...
                        <ConfirmationDialog
                          ...
                          trigger={
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-7 px-2 text-rose-600 hover:bg-rose-50 text-xs font-medium gap-1"
